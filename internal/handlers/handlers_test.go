...
//holds what we posting to a page
...
//a var for the actual test
//slice of struct because we'll have more than 1 test to run
var theTests = []struct {
	name               string //name of a  particular test
	url                string //path matched by the routes
	method             string //GET or post method
	expectedStatusCode int
}{
	{"home", "/", "GET", http.StatusOK},
	{"about", "/about", "GET", http.StatusOK},
	{"general suite", "/generals", "GET", http.StatusOK},
	{"presidential suite", "/majors", "GET", http.StatusOK},
	{"standard rooms", "/standard", "GET", http.StatusOK},
...
func TestHandlers(t *testing.T) {
	routes := getRoutes() //initializing the Routes from the setup file
	...
	for _, e := range theTests {
		if e.method == "GET" {
			resp, err := ts.Client().Get(ts.URL + e.url) //ts.URL calls the web server  e.url appends the url to the server
			if err != nil {
				t.Log(err)
				t.Fatal(err)
			}
...
func TestRepository_Reservation(t *testing.T) {
	reservation := models.Reservation{
		...
		Room: models.Room{
			ID:       1,
			RoomName: "General Suite",
		},
	}
	//putting request it in the session
	req, _ := http.NewRequest("GET", "/make-reservation", nil)

	//getting the context into the request(//putting it in the session)
	ctx := getCtx(req)
	req = req.WithContext(ctx)

	//this returns an initialized response recorder,it simulates when someone passes a request and the response writer writes to the user
	rr := httptest.NewRecorder()
	//putiing reservation in the session
	session.Put(ctx, "reservation", reservation)
	//call the reservation function which is a handler
	handler := http.HandlerFunc(Repo.Reservation)
	handler.ServeHTTP(rr, req) //this calls the reservation handler without using any routes
